#command
# python detect_mask_image.py --image images/pic2.jpeg
...
def mask_image():
	...
	# constructing the argument parser and passing the arguments
	...
	# load the input image from disk, and getting the image dimensions
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			# set the class label and color use to draw the bounding box and text
			label = "Mask" if mask > withoutMask else "No Mask"
			color = (0, 255, 0) if label == "Mask" else (0, 0, 255)

			# display probability in the label
